...
impl Database {
    ...
    pub fn run_migrations(conn: &Connection) -> Result<(), AtomicCoreError> {
        ...
        // Legacy databases from earlier versions may carry seeded rows whose
        // values match the registry — the resolver in `lib.rs` ignores those
        // (treats per-DB row as override only when the value differs from the
        // registry default), so no migration is needed to clean them up.
